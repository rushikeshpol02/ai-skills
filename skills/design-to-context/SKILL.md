...
3. **If Figma MCP is available — ALWAYS delegate to a subagent:**
   ...
   **a) The Figma URL(s)** and how to parse them:
   ...
   - **Also pass the original URL verbatim** — the subagent must use it in the `**Source:**` field (e.g., `https://www.figma.com/design/GPiSba9PGgk2wKyNVO1jMU/MyConnect-2026?node-id=38009-553`). The parsed `fileKey`/`nodeId` are only for making API calls — they must NEVER appear in the source citation.
