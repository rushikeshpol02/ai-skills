...
3. **Artifacts, not chat tables.** Change set and manifest must be written to files under `_runs/update-[YYYYMMDD]/`; chat only carries summary + confirmation gates.
...
At T0 (before Step 1), determine the run folder path:

1. Look for a `_runs/` folder by walking up from the location of the target documents.
2. If found, set run folder to `_runs/update-[YYYYMMDD]/`.
3. If not found, ask the user where to save run artifacts before proceeding.

Use this path for all artifacts below. Never write run artifacts into the same folder as the target documents.

## Required Run Artifacts and RunState Contract

Every run creates and maintains in the run folder:
...
**T0 Gather artifact:** Before Step 1, write `_runs/update-[YYYYMMDD]/Input-Inventory-[YYYYMMDD-HHMM].md` with: available inputs, missing inputs, candidate target documents, mode rationale. `T0` is only completed after this file is written. Update RunState with artifact path and `current_task = T1`.
...
Write the full structured change set to `_runs/update-[YYYYMMDD]/ChangeSet-[YYYYMMDD].md`. Never print the full table in chat.
...
Write the manifest to `_runs/update-[YYYYMMDD]/Manifest-[YYYYMMDD].md`.
...
3. **Per NON-NEGOTIABLE #3:** Change set and manifest are files under `_runs/update-[YYYYMMDD]/`, not chat tables.
