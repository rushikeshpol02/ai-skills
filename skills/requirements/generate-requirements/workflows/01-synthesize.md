...
**In-scope grouping rule:** When extracting in-scope items, group them by user-facing capability — something the user can **do** or **experience** — not by system function or UI element. A capability is the parent; system behaviors and UI components are children.

Example grouping (not flat list):
- Sign In capability → [sign-in form, password recovery link, SSO button]
- Language Selection → [language selector, display language persistence]
- Error Recovery → [error messages, retry mechanism]

This grouping carries forward into the Scope table in `02-generate.md`. Do NOT list atomic UI components (buttons, banners) as top-level scope items.
