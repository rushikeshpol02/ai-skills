...
- The capability has a user goal that cannot be stated as a branch or constraint of any existing FR
- The capability is initiated by a different actor
- The capability has a different entry point or trigger that is not a sub-path of an existing flow
- The capability involves distinct error handling or edge case behavior that warrants independent specification
